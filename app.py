...
ESTADOS_PERMITIDOS = set(
    os.environ.get("ESTADOS_PERMITIDOS", "ES,MG").upper().split(",")
)
...
def transferir_pedido_omie(codigo_pedido_origem):
    ...
    cli_info = chamar_omie(
        OMIE_CLIENTE_URL, "ConsultarCliente",
        APP_KEY_ORIGEM, APP_SECRET_ORIGEM,
        {"codigo_cliente_omie": id_origem_cliente}
    )
    estado_cliente = cli_info.get("estado", "").upper().strip()
    if ESTADOS_PERMITIDOS and estado_cliente not in ESTADOS_PERMITIDOS:
        print(f"Pedido ignorado: cliente no estado [{estado_cliente}] "
